class Dashboard::SuperAdmin::Application::FeatureFlagController < SuperAdminController
  ...
  def show_users
    begin
      ...
      if feature_id.nil?
        ...
        features = Feature.all.where(application_id: params[:application_id]).pluck(:id)
        # Get all user 
        all_user = User.all.where(application_id: params[:application_id]).pluck(:id)
        # Get user that have feature flag
        user_have_feature_flag = UserFeature.where("feature_id IN (?)", features).pluck(:user_id).to_a
