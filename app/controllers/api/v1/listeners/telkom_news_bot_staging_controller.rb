...
class Api::V1::Listeners::TelkomNewsBotStagingController < ApplicationController
  ...
  HELPDESK_QISCUS_EMAIL = 'userid_23_6285868123123@kiwari-stag.com'

  def create
    begin
      response = nil
      parameter_data = params[:telkom_news_bot_staging]
      ...
      if message.include?("@helpdesk") && sender[:qiscus_email] != HELPDESK_QISCUS_EMAIL
        ...
        # response = QiscusAI.telkom_news_bot("staging", sender[:qiscus_id], sender[:qiscus_email], sender[:fullname], qiscus_room_id, message)
        response = QiscusAI.telkom_news_bot_raw_json("staging", parameter_data)
      end
